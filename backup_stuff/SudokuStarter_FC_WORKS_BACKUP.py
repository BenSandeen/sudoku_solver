...
def FC(initial_board,poss_vals):
    ...
    if not poss_vals:
        for row in xrange(size):
            for col in xrange(size):
                poss_vals[row,col] = [] #is_legit
        # localizes append (when applied to poss_vals), saving time each call
        # appender = poss_vals.append
    ...
    # print(poss_vals)
    # print(sorted(poss_vals))
    # for key in sorted(poss_vals):
    #     print(poss_vals[key])
    # print(BoardArray)
                    ...
                    for n in xrange(1,size+1):
                        if is_legit(initial_board,row,col,n):
                            ...
                            if valid:
                                ...
                                # validator(newBoard)
                                # print('failed')
                                # print('poss_vals: ',poss_vals[row,col])
                            ...
                            # print('poss_vals: ',poss_vals[row,col])
                            # # print('n:',n)
                            # removed = [x for x in poss_vals[row,col] if x!=n]#.remove(n)
                            # print('poss_vals: ',poss_vals[row,col])
                            # print('removed: ',removed)
                            # poss_vals[row,col] = [n]
...
def validator(board,poss_vals):#,row,col)
    ...
    for row in xrange(size):
        for col in xrange(size):
            if not BoardArray[row][col]: # if the square's val == 0
                # if poss_vals[row,col]
                isValid = False
                # for pv in poss_vals[row,col]:
                    # print(is_legit(board,row,col,pv))
                # legit = [is_legit(board,row,col,x) for x in poss_vals[row,col]]
                # if not legit:
                #     board.print_board()
                isValid = bool(sum([is_legit(board,row,col,x) for x in poss_vals[row,col]]))
